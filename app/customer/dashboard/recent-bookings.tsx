...
export default function RecentBookingsList() {
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-[2rem] p-5 sm:p-6 flex flex-col gap-4 sm:gap-5 shadow-[0_10px_30px_rgba(0,0,0,0.01)]">
      <div className="flex items-center justify-between">
        <h3 className="font-extrabold text-base lg:text-lg text-zinc-900 dark:text-white select-none">Recent Bookings</h3>
        <Link 
          href="/customer/bookings" 
          className="text-xs sm:text-sm font-bold text-violet-600 hover:text-violet-700"
        >
          View All
        </Link>
      </div>
      ...
      {isLoading && displayBookings.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center p-8 border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl text-center select-none">
          ...
          <span className="text-sm sm:text-base font-bold text-zinc-700 dark:text-zinc-300">
            No bookings found
          </span>
          <span className="text-xs text-zinc-400 dark:text-zinc-500 mt-1 max-w-[240px]">
            You haven't booked any trips yet. Start planning your next trip!
          </span>
          <Link
            href="/customer/book"
            className="mt-4 px-4 py-2 bg-gradient-to-r from-[#ff7c52] to-[#ff2d88] text-white text-xs sm:text-sm font-extrabold rounded-xl shadow-md hover:opacity-90 active:scale-95 transition-all duration-200"
          >
            Book a Trip
          </Link>
        </div>
        ...
        <div className="flex flex-col gap-3">
          {displayBookings.map((booking) => (
            <div 
              key={booking.id}
              className="flex items-center justify-between p-3.5 sm:p-4 border border-zinc-100 dark:border-zinc-800/50 rounded-2xl hover:bg-zinc-50 dark:hover:bg-zinc-800/20 transition-colors duration-200 group cursor-pointer"
            >
              {/* Route & Date */}
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div className="flex flex-col">
                  <span className="text-xs sm:text-sm font-bold text-zinc-800 dark:text-zinc-100 flex items-center gap-1.5 leading-none">
                    {booking.source} <span className="text-zinc-400 font-medium">→</span> {booking.destination}
                  </span>
                  <span className="text-[11px] sm:text-xs text-zinc-400 font-semibold mt-1.5 block leading-none">
              ...
              <div className="flex items-center gap-3 sm:gap-4">
                <div className="flex flex-col text-right">
                  ...
                  <span className="inline-flex mt-1.5 px-2.5 py-0.5 rounded-full text-[10px] sm:text-xs font-bold bg-emerald-500/10 text-emerald-600 dark:bg-emerald-950/20 dark:text-emerald-400 uppercase tracking-wide self-end">
                    {booking.status}
                  </span>
                </div>
