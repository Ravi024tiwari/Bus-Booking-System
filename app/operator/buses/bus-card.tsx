    ...
    >
      ...
      <div className="relative h-38 sm:h-42 w-full rounded-2xl overflow-hidden select-none bg-zinc-100 dark:bg-zinc-800 border border-zinc-150 dark:border-zinc-800/80 shadow-xs">
        ...
        <div className="absolute top-2.5 left-2.5">
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] sm:text-xs font-black tracking-wider uppercase backdrop-blur-md border border-white/20 shadow-xs ${
            isTravelling 
              ? 'bg-emerald-500/20 text-emerald-300 border-emerald-400/40' 
              : 'bg-amber-500/20 text-amber-300 border-amber-400/40'
          }`}>
            <span className={`h-2 w-2 rounded-full ${isTravelling ? 'bg-emerald-400 animate-pulse' : 'bg-amber-400'}`} />
        ...
        <div className="absolute top-2.5 right-2.5">
          <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-black/60 backdrop-blur-md border border-white/20 text-white font-bold text-[10px] sm:text-xs uppercase tracking-wider shadow-xs">
            {bus.type}
          </span>
        </div>
        ...
        <div className="absolute bottom-2.5 left-2.5 flex flex-col text-white">
          <span className="text-xs sm:text-sm font-black tracking-tight drop-shadow-sm flex items-center gap-1.5">
            <Bus className="h-3.5 w-3.5 text-[#ff2d88]" />
            {bus.capacity} Seats
          </span>
          <span className="text-[11px] sm:text-xs text-zinc-200 font-semibold drop-shadow-sm flex items-center gap-1 mt-0.5">
        ...
        {bus.amenities && bus.amenities.length > 0 && (
          <div className="absolute bottom-2.5 right-2.5">
            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-black/60 backdrop-blur-md border border-white/15 rounded-lg shadow-xs">
              {bus.amenities.slice(0, 3).map((amenity) => renderAmenityIcon(amenity))}
              {bus.amenities.length > 3 && (
                <span className="text-[10px] text-white font-bold">+{bus.amenities.length - 3}</span>
              )}
            </div>
          </div>
        )}
      ...
      <div className="flex flex-col gap-2.5 px-0.5">
        ...
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center justify-between gap-1.5">
            <span className="font-black text-base sm:text-lg text-zinc-900 dark:text-white uppercase tracking-tight truncate">
              {bus.busNumber}
            </span>
          ...
          {isTravelling && bus.activeTrip ? (
            <div className="flex items-center gap-1.5 text-xs sm:text-sm font-bold text-indigo-600 dark:text-indigo-400 truncate">
            ...
            <span className="text-xs text-zinc-400 dark:text-zinc-500 font-medium truncate flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-2 gap-2.5 bg-zinc-50 dark:bg-zinc-850/60 border border-zinc-100 dark:border-zinc-800/80 rounded-xl p-2.5 text-xs select-none">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] sm:text-xs font-bold text-zinc-800 dark:text-zinc-200 truncate leading-tight">
                {bus.sleeperSeats && bus.sleeperSeats.length > 0 ? `${bus.sleeperSeats.length} Sleepers` : 'All Seater'}
              </span>
              <span className="text-[10px] text-zinc-400 truncate mt-0.5 leading-tight">
          ...
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              <span className="text-[11px] sm:text-xs font-bold text-indigo-600 dark:text-indigo-400 truncate leading-tight">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-zinc-100 dark:border-zinc-800/60">
          <div className="flex flex-col">
            <span className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider leading-none">Status</span>
            <span className="text-xs sm:text-sm font-bold text-zinc-900 dark:text-white mt-0.5 leading-none">
          ...
          <button
            type="button"
            className="flex items-center gap-1.5 px-3.5 py-2 bg-zinc-900 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-900 font-bold text-xs rounded-xl shadow-xs transition-all cursor-pointer group-hover:bg-[#ff2d88] group-hover:text-white dark:group-hover:bg-[#ff2d88] dark:group-hover:text-white active:scale-95"
          >
