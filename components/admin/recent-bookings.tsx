  ...
  return (
    <div className="bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 rounded-[2rem] p-6 shadow-[0_10px_30px_rgba(0,0,0,0.01)] flex flex-col justify-between h-[360px] group">
      ...
      <div className="flex items-center justify-between pb-3.5 border-b border-zinc-150 dark:border-zinc-800">
        <h3 className="font-extrabold text-base lg:text-lg text-zinc-900 dark:text-white">Recent Bookings</h3>
        <Link 
          href="/admin/bookings" 
          className="text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 font-bold flex items-center gap-0.5 hover:underline"
        >
      ...
      {/* Bookings Table */}
      <div className="flex-1 overflow-x-auto mt-3.5 pr-1">
        <table className="w-full text-left border-collapse min-w-[500px]">
          <thead>
            <tr className="border-b border-zinc-100 dark:border-zinc-800 text-[11px] sm:text-xs text-zinc-400 dark:text-zinc-500 font-bold uppercase select-none">
          ...
          <tbody className="divide-y divide-zinc-100/50 dark:divide-zinc-800/30 text-xs sm:text-sm font-semibold text-zinc-700 dark:text-zinc-300">
            {bookings.map((booking) => {
              ...
              return (
                ...
                >
                  <td className="py-3 pl-1 font-bold text-zinc-900 dark:text-white">
                    {booking.pnr}
                  </td>
                  <td className="py-3">
                  ...
                  <td className="py-3 text-zinc-900 dark:text-white font-extrabold">
                    ₹{booking.amount.toLocaleString('en-IN')}
                  </td>
                  <td className="py-3 text-right pr-1">
                    <span className={`inline-block px-2.5 py-1 border text-[10px] sm:text-xs font-black rounded-full uppercase leading-none ${getStatusStyle(booking.status)}`}>
      ...
      <div className="pt-3 border-t border-zinc-150 dark:border-zinc-800 shrink-0">
        <Link 
          href="/admin/bookings" 
          className="text-xs sm:text-sm text-indigo-600 hover:text-indigo-700 font-extrabold flex items-center justify-center gap-1 group-hover:gap-1.5 transition-all duration-200"
        >
          View all {bookings.length}+ platform transactions
